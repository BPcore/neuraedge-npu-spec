...
_Last updated: 2025-08-22 (Coverage functional TB refinement: direction fairness gate met (fairness≈1.85), arbitration & grant diversity uplift (unique_grant_patterns=29), per‑mode natural saturation progress (natural_bin3_mode_hit=[1,1,1,0]) with adaptive mop-up (per-mode threshold lowering) added; new KPIs (contention_intensity, saturation_ratio, mode_density_unhit, natural_bin3 trackers) exported & CI gating expanded (MAX_DIRECTION_FAIRNESS, MIN_UNIQUE_GRANT_PATTERNS, MAX_MODE_DENSITY_UNHIT); remaining gaps: final natural bin3 mode & active_pe_count_max ceiling (4 vs target 8) blocking full natural saturation)._ 
